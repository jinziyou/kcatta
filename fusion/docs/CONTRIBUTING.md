...
```
主机域  domain crate (fusion-asset, fusion-malware)
          → fusion-runtime (Collector, run_scan_at) → fusion-contract (AssetReport)
网络域  fusion-flow (capture + intel) → fusion-contract (FlowBatch)
共享    fusion-ingest 上报 AssetReport / FlowBatch → form
```

**原则**：fusion 只采集；CVE / 漏洞识别与跨源关联在 form 侧完成。
...
`fusion-asset` 对外仍按**资产语义**暴露 `Collector`（Host / Packages / …），内部按**采集策略**分层：
...
1. 在 `fusion-flow/src/intel/sync/` 实现 feed 适配器（参考 `feodo.rs`）
...
| Rust 镜像 | `fusion-contract/src/lib.rs` |
| 校验 | `cargo test -p fusion-runtime` |
...
| 契约测试 | `fusion-runtime/tests/contract.rs` | 需 `form/schemas-json/` 存在 |
| 集成测试 | `fusion-remote/tests/` | 默认忽略；见 remote README |

Fixture 扫描根目录写法见 `fusion-runtime/tests/fixture.rs`。

## 二进制与 feature

| 二进制 | crate | 说明 |
| --- | --- | --- |
| `fusion-asset` | fusion-asset | 静态分文件扫描 |
| `fusion-malware` | fusion-malware | ClamAV → `malware.json` |
| `fusion-host` | fusion-host-cli | 合并 `AssetReport` |
| `fusion-remote` | fusion-remote | SSH agent 模式 |

`fusion-host-cli` features：`asset`（默认）、`malware`、`ingest`、`full`。
