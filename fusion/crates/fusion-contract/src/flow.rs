//! Network flow contract — the `fusion-flow` (network collector) envelope.
//!
...
//! shared `fusion-ingest` client can serialize both host and network telemetry,
...
/// One IOC hit observed on a flow by fusion-flow's preliminary processing.
...
pub struct FlowEvent {
    ...
    /// IOC matches found by fusion-flow's preliminary processing.
    /// Serialized as `[]` when empty so the field is always present.
    #[serde(default)]
    pub threat_intel: Vec<ThreatMatch>,
}

/// fusion-flow -> form: a batch of flow events from one collector instance.
#[derive(Debug, Clone, Serialize, Deserialize)]
