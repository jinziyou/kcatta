//! `fusion flow`: capture one cycle, apply threat-intel IOC matching, then emit
//! and/or upload the resulting `FlowBatch`.
...
pub fn run(args: FlowArgs) -> Result<()> {
    ...
    if args.list_devices {
        let names = fusion_flow::pcap::list_devices().context("list pcap devices")?;
        for name in names {
            println!("{name}");
        }
        return Ok(());
    ...
    if let Some(base) = &args.upload {
        fusion_ingest::upload_batch(&batch, base).context("uploading batch")?;
