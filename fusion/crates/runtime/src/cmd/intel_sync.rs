//! `fusion intel-sync`: download remote IOC feeds to local JSON.
//!
//! Mirrors the offline-friendly refresh model of `form-osv-sync`: sync is an
//! explicit, schedulable step; `fusion flow --intel` reads the on-disk feed
...
use clap::Args;
use fusion_flow::intel::sync::{self, feodo};
use fusion_flow::ThreatFeed;
