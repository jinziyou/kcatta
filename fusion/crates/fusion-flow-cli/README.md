...
[`fusion-flow`](../fusion-flow/README.md) 的命令行驱动（bin: **`fusion-flow`**）：跑一轮捕获 → 威胁情报匹配 → 输出 / 上报 [`FlowBatch`](../fusion-contract/src/lib.rs)。
...
sudo cargo run -p fusion-flow-cli --features pcap -- \
...
`fusion-flow`（捕获 + 匹配）+ `fusion-ingest`（上报）。情报库同步见 [`fusion-intel-sync`](../fusion-intel-sync/README.md)，用法速查见 [`../../README.md`](../../README.md)。
