...
use anyhow::Context;
use fusion_runtime::{Collector, CollectorOutput, ScanContext, WindowsPackageProfile};
    ...
    pub fn parse(s: &str) -> anyhow::Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            "ports" | "port" => anyhow::bail!(
                "port scanning is not supported by fusion-asset (use host|packages|sbom|identity|all)"
            ),
            other => {
                anyhow::bail!(
