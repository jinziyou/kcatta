...
Ships a **static `fusion-asset` binary** to a
...
```bash
# 0. build a STATIC agent binary once. fusion-asset bundles SQLite (C), so pick one:
...
#    -> target/x86_64-unknown-linux-musl/release/fusion-asset  (the default --asset-binary)

#    option B — static glibc via the native gcc (no extra C toolchain); then pass
#    --asset-binary target/x86_64-unknown-linux-gnu/release/fusion-asset
...
cargo run -p fusion-remote -- \
    --ssh-host root@10.22.0.243 --target host --output ./reports/10.22.0.243
```

The managed key lives at
`~/.config/scdr/fusion-remote/keys/<user>@<host>-<port>.ed25519`.
...
scp fusion-asset (static musl) ─────▶ <workdir>/fusion-asset
                                       sha256 verify
ssh exec fusion-asset -r / -t … ────▶ writes <workdir>/out/*.json
...
### Scanner host (where you run `fusion-remote`)

| Tool | Purpose |
| --- | --- |
| `ssh` / `scp` (OpenSSH) | Control + transfer, multiplexed via `ControlMaster` |
| `ssh-keygen` | Generate the managed ed25519 key on first run |
| Rust toolchain + a static build | Build the static `fusion-asset` to ship — musl target (`rustup target add x86_64-unknown-linux-musl` **plus a musl C compiler** for the bundled SQLite), or static-glibc via the native gcc (`-C target-feature=+crt-static`) |
...
fusion-remote \
    ...
    --asset-binary target/x86_64-unknown-linux-musl/release/fusion-asset \
    --target all \
...
| `--asset-binary` | transport-specific | musl Linux binary (SSH) or `fusion-asset.exe` (WinRM) |
| `--scan-root` | `/` or `C:\` | Filesystem root on the target |
| `--windows-packages` | `apps` | `full` (include CBS updates) or `apps` (skip CBS noise) |
| `--winrm-password` | — | WinRM password (`FUSION_WINRM_PASSWORD` env); falls back to `--ssh-password` |
...
| `--malware` | off | Also run `fusion-malware` on the target (needs `clamd` there) |
| `--malware-binary` | musl `fusion-malware` | Static binary shipped when `--malware` is set |
...
# -> target/x86_64-pc-windows-msvc/release/fusion-asset.exe
```

Run from a Linux or Windows scanner host with PowerShell (`pwsh` or `powershell`) on PATH:

```bash
FUSION_WINRM_PASSWORD='...' cargo run -p fusion-remote -- \
    --transport winrm \
...
with a Windows-built `fusion-asset.exe` (same upload/exec pipeline as Linux).
...
```bash
fusion-remote --ssh-host root@10.22.0.243 --revoke-key
```
...
managed keypair under `~/.config/scdr/fusion-remote/keys/`. It is a no-op if the
...
- Unit tests: `cargo test -p fusion-remote`.
- Real-target bootstrap (ignored by default):

```bash
SCDR_TEST_TARGET=user@host SCDR_SSH_PASSWORD=... \
    cargo test -p fusion-remote --test integration_bootstrap -- --ignored --nocapture
```
