...
//!
//! Each collector implements [`fusion_runtime::Collector`] or exposes a
...
pub fn collect_services(ctx: &ScanContext) -> Vec<fusion_contract::Asset> {
    services::collect(ctx)
}

/// Collect local account assets discovered under `ctx.scan_root`.
pub fn collect_accounts(ctx: &ScanContext) -> Vec<fusion_contract::Asset> {
    accounts::collect(ctx)
}

/// Collect credential assets discovered under `ctx.scan_root`.
pub fn collect_credentials(ctx: &ScanContext) -> Vec<fusion_contract::Asset> {
