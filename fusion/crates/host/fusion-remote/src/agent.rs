//! Agent-mode remote scan: ship a static `fusion-asset` binary to the
...
//! 6. Run `fusion-asset -r <root> -t <target> -o <work>/out`.
...
pub struct MalwareAgentOptions {
    /// Local static `fusion-malware` binary to ship.
    pub binary: PathBuf,
...
pub struct AgentScanOptions {
    ...
    /// Local static `fusion-asset` binary to ship (musl recommended).
    pub asset_binary: PathBuf,
    /// Filesystem root to scan on the target (default `/`).
    pub scan_root: String,
    /// What to collect (`host`, `all`, …) — forwarded to remote `fusion-asset -t`.
    pub target: ScanTarget,
    /// Local output directory for the per-asset JSON files.
    pub output_dir: PathBuf,
    /// Optional stable task id; auto-generated if `None`.
    pub task_id: Option<String>,
    /// When set, ship and run `fusion-malware` on the target (needs `clamd` there).
...
pub fn run_agent_scan(mut opts: AgentScanOptions) -> anyhow::Result<AgentScanReport> {
    ...
    let remote_bin = format!("{}/fusion-asset", workdir.path());
    let remote_out = format!("{}/out", workdir.path());

    session
        .scp_upload(&opts.asset_binary, &remote_bin)
        .context("upload fusion-asset binary")?;
...
fn run_remote_malware(
    ...
    if !malware.binary.is_file() {
        bail!(
            "malware binary not found: {}\n\
             build it first:\n  \
             rustup target add x86_64-unknown-linux-musl\n  \
             cargo build -p fusion-malware --target x86_64-unknown-linux-musl --release",
            malware.binary.display()
        );
    }

    let remote_bin = format!("{}/fusion-malware", workdir.path());
    session
        .scp_upload(&malware.binary, &remote_bin)
        .context("upload fusion-malware binary")?;
    ...
    if exit != Some(0) {
        bail!(
            "remote fusion-malware failed (exit {:?})\nstdout: {}\nstderr: {}",
            exit,
            run.stdout.trim(),
            run.stderr.trim()
        );
...
impl Drop for RemoteWorkdir<'_> {
    fn drop(&mut self) {
        ...
        if self.path.starts_with('/') && self.path.contains("/scan-") {
            ...
            {
                eprintln!(
                    "[fusion-remote/agent] cleanup rm -rf {} failed: {e:#}",
                    self.path
                );
            }
...
fn verify_upload(session: &SshSession, local: &Path, remote_path: &str) -> anyhow::Result<()> {
    ...
    if remote_sum.is_empty() {
        ...
        eprintln!(
            "[fusion-remote/agent] sha256sum unavailable on target; skipping integrity check"
        );
        return Ok(());
    }
