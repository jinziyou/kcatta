...
impl WinRmSession {
    ...
    fn invoke_raw(&self, ps_script: &str) -> Result<CommandOutput> {
        ...
        cmd.arg("-NoProfile")
            ...
            .arg("-Command")
            .arg(&wrapper)
            .env("FUSION_WINRM_HOST", &self.opts.host)
            .env("FUSION_WINRM_USER", &self.opts.user)
            .env("FUSION_WINRM_PASSWORD", &self.opts.password)
            .env("FUSION_WINRM_PORT", self.opts.port.to_string())
            .env(
                "FUSION_WINRM_USE_SSL",
                if self.opts.use_ssl { "1" } else { "0" },
            )
            .env(
                "FUSION_WINRM_SKIP_CERT",
                if self.opts.skip_cert_check { "1" } else { "0" },
            )
            .env("FUSION_WINRM_SCRIPT_B64", script_b64);
...
$HostName = $env:FUSION_WINRM_HOST
$User = $env:FUSION_WINRM_USER
$Password = ConvertTo-SecureString $env:FUSION_WINRM_PASSWORD -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential($User, $Password)
$Port = [int]$env:FUSION_WINRM_PORT
$UseSSL = $env:FUSION_WINRM_USE_SSL -eq '1'
$SkipCert = $env:FUSION_WINRM_SKIP_CERT -eq '1'
...
try {
  $Block = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($env:FUSION_WINRM_SCRIPT_B64))
