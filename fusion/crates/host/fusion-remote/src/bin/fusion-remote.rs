//! `fusion-remote`: agent-mode remote scanner front-end.
//!
//! Ships a static `fusion-asset` to the target over SSH or WinRM, runs it in place,
...
use fusion_ingest::upload_report;
use fusion_malware::default_workers;
use fusion_remote::{
...
#[command(
    name = "fusion-remote",
    version,
    about = "posture remote scanner: ship fusion-asset, run on target, pull JSON back"
)]
struct Args {
    ...
    /// `~/.config/scdr/fusion-remote/keys/` is used (auto-generated).
    ...
    /// WinRM password (`FUSION_WINRM_PASSWORD` env). Falls back to `--ssh-password`
    /// when unset. Required for `--transport winrm`.
    #[arg(
        long,
        value_name = "PWD",
        env = "FUSION_WINRM_PASSWORD",
        hide_env_values = true
    )]
    ...
    /// Scan target forwarded to fusion-asset: `host` | `packages` | `sbom` |
    ...
    /// Local static `fusion-asset` binary to ship.
    ...
    /// Local static `fusion-malware` binary to ship when `--malware`.
    #[arg(
        long,
        value_name = "PATH",
        default_value = "target/x86_64-unknown-linux-musl/release/fusion-malware"
    )]
...
fn default_asset_binary(transport: Transport) -> PathBuf {
    match transport {
        Transport::Ssh => PathBuf::from("target/x86_64-unknown-linux-musl/release/fusion-asset"),
        Transport::Winrm => PathBuf::from("target/x86_64-pc-windows-msvc/release/fusion-asset.exe"),
    }
}
...
fn resolve_winrm_password(
    ...
    anyhow::bail!(
        "WinRM password required: set FUSION_WINRM_PASSWORD, pass --winrm-password, \
...
fn revoke_managed_key(
    ...
    let removed = fusion_remote::bootstrap::revoke_key(ssh_host, ssh_port, ssh_identity, password)
    ...
    if ssh_identity.is_none() {
        match fusion_remote::bootstrap::managed_key_path(ssh_host, ssh_port) {
            Ok(priv_key) => {
