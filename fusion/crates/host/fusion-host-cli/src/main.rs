//! fusion-host-cli: assemble a scan plan, run it, emit JSON (optionally upload).

use std::path::PathBuf;

use anyhow::{Context, Result};
use clap::Parser;
use fusion_runtime::{run_scan_at_with_opts, Collector, WindowsPackageProfile};
...
struct Args {
    ...
    #[cfg(feature = "malware")]
    #[arg(long, default_value_t = fusion_malware::default_workers())]
    malware_jobs: usize,
}
...
fn build_plan(
    ...
    #[cfg(feature = "asset")]
    plan.extend(fusion_asset::default_collectors());

    #[cfg(feature = "malware")]
    plan.push(Box::new(
        fusion_malware::MalwareCollector::default().with_workers(args.malware_jobs),
    ));
...
fn main() -> Result<()> {
    ...
    let scan_root = args
        .root
        .clone()
        .unwrap_or_else(fusion_asset::platform::default_scan_root);

    let windows_packages = WindowsPackageProfile::parse(&args.windows_packages)?;

    #[cfg(feature = "asset")]
    if let Some(out_dir) = &args.asset_out {
        let target = fusion_asset::ScanTarget::parse(&args.target)?;
        let options = fusion_asset::ScanOptions {
            root: scan_root.clone(),
            target,
            project_roots: args.project_root.clone(),
            windows_packages,
        };
        let written = fusion_asset::run_static_scan(&options, out_dir).context("static scan")?;
