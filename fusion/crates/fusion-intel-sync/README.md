...
把远程威胁情报 IOC feed 下载为本地 JSON，供 [`fusion-flow-cli`](../fusion-flow-cli/README.md) `--intel` 离线匹配（bin: **`fusion-intel-sync`**）。
...
新增情报源：在 [`fusion-flow/src/intel/sync/`](../fusion-flow/src/intel/sync/) 实现适配器（参考 `feodo.rs`），再接入 `--source` 分发。详见 [`../../docs/CONTRIBUTING.md`](../../docs/CONTRIBUTING.md)「新增情报源（网络域）」。
