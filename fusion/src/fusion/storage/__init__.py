"""Persistence backends used by fusion's ingest pipeline."""
...
def create_store(
    ...
    *,
    ...
    1. ``backend`` argument
    2. ``FUSION_STORAGE`` env (``jsonl`` default, or ``sqlite``)
    """
    name = storage_backend_name(backend)
    if name == "sqlite":
        return SqliteStore(data_dir / "fusion.db", _SQLITE_TABLES[kind])
    if name != "jsonl":
        msg = f"unknown FUSION_STORAGE backend {name!r} (want jsonl or sqlite)"
