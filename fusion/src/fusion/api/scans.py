...
The portal calls these to register targets and trigger remote scans. A trigger
creates a `ScanJob` (pending), schedules an async background runner that deploys
the agent via `fusion.deploy.trigger` (off the event loop with
...
def _public_url() -> str:
    """fusion URL the guard daemon on a target should push events to."""
    return os.getenv("FUSION_PUBLIC_URL", "http://127.0.0.1:8000")
...
async def register_target(payload: ScanTargetInput, request: Request) -> ScanTarget:
    ...
    a managed key on the fusion host and is then discarded — never persisted."""
