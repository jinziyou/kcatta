...
run ``posture-host`` against ``C:\\``, pull the per-asset JSON back (base64 over
...
def run_winrm_agent_scan(opts: WinRmAgentScanOptions):
    ...
    if not opts.agent_binary.is_file():
        raise FileNotFoundError(
            ...
            "build it first: cargo build -p posture-host "
    ...
    try:
        remote_bin = f"{workdir}\\posture-host.exe"
        ...
        if parse_marked_exit(stdout) != 0:
            raise RuntimeError(
                f"remote posture-host failed (exit {parse_marked_exit(stdout)})\n"
