...
# Scan target -> posture-host argument (`posture-host -t <arg>`).
...
# Accepted `--windows-packages` profiles (`posture-host --windows-packages <p>`).
WINDOWS_PACKAGE_PROFILES: tuple[str, ...] = ("full", "apps")

# Per-target per-asset JSON files written by `posture-host -o DIR`.
...
def expected_files(target: str) -> tuple[str, ...]:
    """JSON files `posture-host -t <target> -o DIR` is expected to produce."""
    try:
