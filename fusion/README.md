...
**数据分析与态势感知平台**，kcatta 的分析核心。基于 Python 构建，负责把 `agent`（主机 + 网络探针）上传的异构数据标准化、做关联分析、打分入库，并对 `portal` 暴露查询接口。
...
- 数据契约共 7 类：**上行采集 envelope** `AssetReport`（`agent-host` → fusion）、`FlowBatch`（`agent-flow` → fusion）、`GuardEventBatch`（`agent-guard` → fusion）；**fusion 派生、对 portal 暴露** `DetectionResult`、`Alert`、`AttackPath`；**外部红队** `CapabilityGraph`（opaque，→ fusion）
...
- **端到端打通**：`agent-host` 与 `agent-flow capture` 的 JSON 输出可以直接 `curl -X POST` 到 fusion 完成入库
...
# flow -> fusion（抓包 + 威胁情报 IOC 匹配 + 上报，一步到位；上报经统一 agent，agent-flow 本身不上报）
...
跨机编排是 fusion 的职责：把 `agent-host` 探针**投放到待测机器**、就地扫描、把分文件
...
# Windows 目标（WinRM；需 pip install 'kcatta-fusion[winrm]' 与 agent-host.exe）
...
- 投放管线（host，默认）：探测 arch → 选可写非 `noexec` 工作目录 → 上传 `agent-host` 并 sha256 校验 →
  ...
  （注：上报由 fusion-scan 自身完成；投放的 `agent-host` 只产出文件、不上报。）
- `--capability flow`：上传 `agent-flow` → 远程 `capture`（`--pcap`/`--iface`/`--duration`/`--bpf` 可选）→ 拉回 `flow.json` → `--upload` 则由 fusion-scan POST `/ingest/flow-batch`。一次性，清理工作目录。
- `--capability guard`：上传 **`agent`** 二进制到持久目录 → `setsid` 后台启动 `agent guard --upload <fusion>` 常驻守护（**不**清理，持续推送；只有 `agent` 会上报，故 guard 投 `agent` 而非 `agent-guard`）；`--guard-config` 可上传本地 `guard.json`。**`--upload` 必填**。
