...
| 变量 | 默认 | 用途 |
| --- | --- | --- |
| `NEXT_PUBLIC_FUSION_BASE_URL` | `http://127.0.0.1:8000` | fusion HTTP API 的基准 URL |
| `FUSION_API_TOKEN` | （未设置） | 服务端调用 fusion API 时携带的 Bearer Token（与 fusion 的 `FUSION_API_TOKEN` 对应）；fusion 无鉴权时留空 |
...
需要先启 fusion（默认 `127.0.0.1:8000`，CORS 已默认放行 `http://localhost:3000`）：
...
如 fusion 跑在不同端口，启动 portal 时覆盖：

```bash
NEXT_PUBLIC_FUSION_BASE_URL=http://127.0.0.1:18000 pnpm dev
```

## 契约约定

- 数据契约的**源头**在 `fusion/src/fusion/schemas/`（Pydantic）。
- 跨语言**派生**在 `fusion/schemas-json/*.schema.json`。
- portal 通过**代码生成**获取契约：`pnpm generate:contracts`（`scripts/generate-contracts.mjs`，基于 `json-schema-to-typescript`）把 `fusion/schemas-json/*.schema.json` 转为 `src/lib/schemas/*.ts`（带「勿手改」banner）；`src/lib/contracts.ts` 统一 re-export 这些生成类型，并补两个派生别名（`Asset` / `AssetKind`）。`make contracts-check` 在 CI 校验生成结果不漂移。
