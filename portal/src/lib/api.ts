...
/** POST `body` as JSON to fusion and parse the JSON response (server-side only). */
async function post<T>(path: string, body: unknown): Promise<T> {
  ...
  if (!response.ok) {
    // Surface fusion's `{detail: ...}` validation/business error when present.
    ...
    throw new FusionApiError(`fusion API ${detail} on ${path}`, response.status);
  }
  return (await response.json()) as T;
}
