    ...
    <FieldGroup>
      ...
      <div className="flex flex-wrap items-center gap-3 border-t pt-4">
        ...
        <span className="text-muted-foreground text-xs">
          下发后 fusion 将远程部署 agent 并采集 {CAPABILITY_META[capability].produces}。
        </span>
      </div>
    </FieldGroup>
  );
...
function CapabilityOptions({
  ...
  return (
    <div className="bg-muted/30 text-muted-foreground flex items-start gap-2 rounded-lg border p-4 text-sm">
      ...
      <p>
        实时防护将在目标上启动 <span className="font-mono">posture-guard</span>{" "}
        常驻守护进程，持续监控并把事件推送回 fusion。任务下发后即视为成功，事件流可在「防护事件」页查看。
      </p>
    </div>
  );
}
