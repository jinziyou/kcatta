r"""WinRM client-certificate bootstrap — the WinRM analog of SSH ``bootstrap``.
...
1. Generate a managed client certificate on the analyzer host
   (``~/.config/scdr/agent-remote/winrm-certs/<user>@<host>-<port>.{crt,key}``).
...
.. warning::
   **Password exposure on the target.** The mapping step needs the local account's
   ...
   / Sysmon 1 / EDR) during bootstrap and rotate. The analyzer side never persists or
...
.. warning::
   The password-bearing session uses ``skip_cert_check`` (relaxed TLS server-cert
   validation), consistent with the project's trusted-lab posture (cf. SSH
   AutoAddPolicy). On an untrusted analyzer→target path a MITM could capture the
   NTLMv2 exchange — only use on networks you control.
"""
...
from .bootstrap import _config_dir, _sanitize  # shared path helpers (XDG-aware)
...
def managed_cert_paths(target: str, port: int) -> tuple[Path, Path]:
    ...
    base = _cert_dir() / f"{_sanitize(user)}@{_sanitize(host)}-{port}"
    return base.with_suffix(".crt"), base.with_suffix(".key")
...
def _generate_client_cert(cert: Path, key: Path, user: str, host: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-subj", f"/CN={user}",
            "-addext", f"subjectAltName=otherName:{_MS_UPN_OID};UTF8:{upn}",
...
def _ps_remove_mapping(upn: str, thumbprint: str) -> str:
    return (
        ...
        f"Where-Object {{ $_.Subject -eq '{_escape_ps(upn)}' }} | "
        "Remove-Item -Force -Recurse -ErrorAction SilentlyContinue; "
        "Get-ChildItem Cert:\\LocalMachine\\Root, Cert:\\LocalMachine\\TrustedPeople "
        "-ErrorAction SilentlyContinue | "
        f"Where-Object {{ $_.Thumbprint -eq '{_escape_ps(thumbprint)}' }} | "
        "Remove-Item -Force -ErrorAction SilentlyContinue; Write-Output __revoked"
    )
...
def _password_session(target: str, port: int, password: str, skip_cert_check: bool) -> WinRmSession:
    user, host = _split(target)
    return WinRmSession(
        WinRmOptions(user=user, host=host, password=password, port=port,
...
def _cert_session(
    ...
    user, host = _split(target)
    return WinRmSession(
        WinRmOptions(user=user, host=host, port=port, cert_pem=cert, cert_key_pem=key,
                     skip_cert_check=skip_cert_check)
    )
...
def ensure_cert_auth(
    ...
    if not can_authenticate_cert(target, port, skip_cert_check=skip_cert_check):
        raise RuntimeError(
...
def rotate_cert(target: str, port: int, password: str | None = None,
                skip_cert_check: bool = False) -> tuple[Path, Path]:
    """Rotate the managed WinRM cert: regenerate + re-bootstrap.
    ...
    provided. The old cert/mapping is replaced by the new bootstrap.
    """
    if not password:
        raise RuntimeError(
    ...
    cert, key = managed_cert_paths(target, port)
    user, host = _split(target)
    _generate_client_cert(cert, key, user, host)  # overwrite in place
    return ensure_cert_auth(target, port, password, skip_cert_check=skip_cert_check)


def revoke_cert(target: str, port: int, password: str | None = None,
                skip_cert_check: bool = False) -> bool:
    ...
    revoked = "__revoked" in _out(result)
...
def _out(resp) -> str:
    return resp.std_out.decode("utf-8", "replace") if isinstance(resp.std_out, bytes) else str(
        resp.std_out
    )
