...
- **生产必须设置强随机 token**，例如：
  ...
  ```
- `docker compose` **没有内置默认 token**：未在 `.env` 设置 `FUSION_API_TOKEN` 会拒绝启动
  （避免“出厂即弱口令”）。`cp .env.example .env` 后填入强随机值。
- 空字符串（`FUSION_API_TOKEN=`）等同未设置 —— agent 上报端也按未设置处理，不会发送空 Bearer 头。

## 网络暴露

- 生产建议把 fusion 绑定到 `127.0.0.1` 并经反向代理（TLS + 鉴权）对外，而非直接 `0.0.0.0:8000`。
- **CORS**：默认仅放行 `http://localhost:3000`；生产用 `FUSION_CORS_ORIGINS` 收敛到真实前端域名。
  允许的请求头已收敛为 `Authorization` / `Content-Type`，且不允许携带 Cookie（鉴权只走 bearer 头）。
- **请求体上限**：ingest 默认限制单请求 ≤ 10MB（`FUSION_MAX_BODY_BYTES` 可调），防止超大上传打满内存/磁盘。
  生产仍建议在反向代理层再设一道 body-size 上限。

## 持久化

- v0 默认 JSONL（单写者）；**生产推荐 SQLite**（`FUSION_STORAGE=sqlite`，docker compose 即用此）。
- 切换后端前先 `fusion-migrate-storage` 迁移历史数据。
...
- 受管密钥落在 `~/.config/scdr/agent-remote/keys/`；用 `fusion-scan --revoke-key` 撤销。

## portal

- fusion token 仅在 portal **服务端**转发（非 `NEXT_PUBLIC_`），不会进入客户端 bundle。
- `NEXT_PUBLIC_FUSION_BASE_URL` 是 **构建期** 注入（Next.js），Docker 下通过 `--build-arg` 传入。
