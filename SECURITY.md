...
- **WinRM 托管凭证（客户端证书）**：与 SSH 同样的「一次性口令 bootstrap、之后免口令」模型——注册时口令用一次（在目标上启用证书认证、导入客户端证书、建 `WSMan ClientCertificate` 映射），随即丢弃，**绝不持久化**；之后用客户端证书（`transport=ssl`）连。客户端证书/私钥落在 `~/.config/scdr/agent-remote/winrm-certs/`（目录 0700、文件 0600）。证书认证仅 HTTPS（前置需目标已有 HTTPS WinRM 监听器）；触发路径默认 `skip_cert_check`（与 SSH AutoAddPolicy 同一信任姿态，仅适用于可信实验/内网）。⚠️ 目标侧 PowerShell **未经真机端到端验证**（仅 mock 单测），详见 `agent/docs/WINDOWS-SUPPORT.md`。
