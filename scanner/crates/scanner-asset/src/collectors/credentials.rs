//! SSH public keys and authorized_keys entries (fingerprints only, never secrets).

use base64::Engine;
use std::fs;
use std::path::Path;

use scanner_contract::{Asset, Credential, CredentialKind};
...
fn scan_home_ssh(ctx: &ScanContext, out: &mut Vec<Asset>) {
...
// Minimal SHA-256 (same approach as scanner-remote agent checksum).
struct Sha256 {
    state: [u32; 8],
    len: u64,
    buf: [u8; 64],
