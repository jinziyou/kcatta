...
pub(crate) fn collect_host(ctx: &ScanContext) -> anyhow::Result<HostInfo> {
    ...
    let hostname =
        read_trim_at(root, "etc/hostname").unwrap_or_else(|| "unknown-host".to_string());
    let os = read_os_release(&join_root(ctx, "etc/os-release")).unwrap_or_else(|| "unknown".to_string());
...
fn read_os_release_key(path: &std::path::Path, key: &str) -> Option<String> {
    let text = std::fs::read_to_string(path).ok()?;
    text.lines()
        .find_map(|line| parse_kv(line, key))
}

fn read_machine_arch(cpuinfo: &std::path::Path) -> Option<String> {
    let text = std::fs::read_to_string(cpuinfo).ok()?;
    text.lines()
        .find_map(|line| line.strip_prefix("machine\t: ").map(str::trim).map(str::to_string))
}
