"""Form WinRM (PowerShell) injection-path tests.
...
def test_winrm_certificate_validation_is_secure_by_default(monkeypatch, value):
    if value is None:
        monkeypatch.delenv(wm.WINRM_SKIP_CERT_CHECK_ENV, raising=False)
    else:
        monkeypatch.setenv(wm.WINRM_SKIP_CERT_CHECK_ENV, value)
    assert wm.winrm_skip_cert_check() is False


@pytest.mark.parametrize("value", ["1", "true", "YES", "on"])
def test_winrm_certificate_validation_can_be_explicitly_disabled(monkeypatch, value):
    monkeypatch.setenv(wm.WINRM_SKIP_CERT_CHECK_ENV, value)
    assert wm.winrm_skip_cert_check() is True


def test_invalid_winrm_certificate_setting_fails_closed(monkeypatch):
    monkeypatch.setenv(wm.WINRM_SKIP_CERT_CHECK_ENV, "sometimes")
    with pytest.raises(ValueError, match=wm.WINRM_SKIP_CERT_CHECK_ENV):
        wm.winrm_skip_cert_check()
...
@pytest.mark.parametrize("bad_target", ["host; Remove-Item C:\\", "not-a-target", "all'$(x)'"])
