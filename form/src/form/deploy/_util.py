...
Ports the small pure helpers from the former Rust ``fusion-remote`` crate
...
# Scan target -> fusion command argument (`fusion host -t <arg>`).
...
# Per-target per-asset JSON files written by `fusion host -o DIR`.
...
def expected_files(target: str) -> tuple[str, ...]:
    """JSON files `fusion host -t <target> -o DIR` is expected to produce."""
    try:
