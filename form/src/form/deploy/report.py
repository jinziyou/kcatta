...
The JSON files written by ``fusion host -o DIR`` mirror form's own Pydantic
contracts, so we validate them directly with :mod:`form.schemas` rather than a
...
def upload_asset_report(report: AssetReport, form_base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"form ingest failed ({exc.code}): {detail}") from exc
    if status != 202:
        raise RuntimeError(f"form ingest returned unexpected status {status}")
