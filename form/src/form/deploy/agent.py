...
Ship a static ``fusion`` binary to the target over SSH, run ``fusion host`` in
...
class MalwareAgentOptions:
    """Also run ClamAV on the target (`fusion host --malware`; needs clamd there)."""
...
class _RemoteWorkdir:
    ...
    def __exit__(self, *_exc: object) -> None:
        ...
        try:
            ...
            print(f"[form.deploy] cleanup rm -rf {self.path} failed: {exc}")
...
def run_agent_scan(opts: AgentScanOptions) -> AgentScanReport:
    ...
    with SshSession(host=host, user=user, key_path=key, port=opts.port) as session:
        ...
        with _RemoteWorkdir(session, task_id) as workdir:
            remote_bin = f"{workdir.path}/fusion"
            remote_out = f"{workdir.path}/out"

            session.upload(opts.fusion_binary, remote_bin)
            ...
            if parse_marked_exit(run.stdout) != 0:
                raise RuntimeError(
                    f"remote fusion host failed (exit {parse_marked_exit(run.stdout)})\n"
...
def _probe_arch_compatible(session: SshSession) -> None:
    ...
    if arch not in ("x86_64", "amd64"):
        raise RuntimeError(
            ...
            "(build a matching `fusion` target)"
        )
...
def _verify_upload(session: SshSession, local: Path, remote_path: str) -> None:
    ...
    if not remote_sum:
        print("[form.deploy] sha256sum unavailable on target; skipping integrity check")
        return
