...
Mirrors the SSH agent pipeline over PowerShell remoting: ship ``fusion.exe``,
run ``fusion host`` against ``C:\\``, pull the per-asset JSON back (base64 over
WinRM), then clean up. Install the extra with ``pip install 'posture-form[winrm]'``.
"""
...
class WinRmSession:
    ...
    def __init__(self, opts: WinRmOptions) -> None:
        try:
            ...
            raise RuntimeError(
                "WinRM transport needs pywinrm — install with: pip install 'posture-form[winrm]'"
            ) from exc
...
def run_winrm_agent_scan(opts: WinRmAgentScanOptions):
    ...
    if not opts.fusion_binary.is_file():
        raise FileNotFoundError(
            f"fusion binary not found: {opts.fusion_binary}\n"
            "build it first: cargo build -p fusion-runtime "
    ...
    try:
        remote_bin = f"{workdir}\\fusion.exe"
        remote_out = f"{workdir}\\out"

        session.upload_file(opts.fusion_binary, remote_bin)
        _verify_upload(session, opts.fusion_binary, remote_bin)
        ...
        if parse_marked_exit(stdout) != 0:
            raise RuntimeError(
                f"remote fusion host failed (exit {parse_marked_exit(stdout)})\n"
...
def _verify_upload(session: WinRmSession, local: Path, remote_path: str) -> None:
    ...
    if not remote_sum:
        print("[form.deploy/winrm] Get-FileHash returned empty; skipping integrity check")
        return
