...
becomes one `Alert` that links *all* the flows (and hosts) that touched
...
from ..schemas import Alert, FlowBatch, IndicatorType, Severity
...
class _Group:
    ...
    def observe(self, flow_id: str, host_id: str, severity: Severity, category: str, source: str):
        ...
        _append_unique(self.flow_ids, flow_id)
...
def _alert_for_group(batch: FlowBatch, group: _Group) -> Alert:
    categories = "/".join(group.categories)
    title = (
        f"{len(group.flow_ids)} flow(s) matched threat indicator {group.indicator} ({categories})"
    )
    description = (
        f"Indicator {group.indicator} ({group.indicator_type.value}, {categories}) was hit by "
        f"{len(group.flow_ids)} flow(s) from host(s) {', '.join(group.host_ids)} "
    ...
    return Alert(
        ...
        related_flow_ids=group.flow_ids,
        created_at=batch.collected_at,
    )


def correlate_flow_batch(batch: FlowBatch) -> list[Alert]:
