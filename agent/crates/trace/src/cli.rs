//! `agent-trace` CLI: subcommands + run, shared by the standalone `agent-trace`
...
fn run_capture_cmd(args: TraceArgs) -> Result<Option<TraceBatch>> {
    ...
    let mut batch = run_capture_with_config(&feed, &capture_config).context("running capture")?;
...
fn http_get_text(url: &str, timeout: Duration) -> Result<String> {
    let client = reqwest::blocking::Client::builder()
        .timeout(timeout)
        .user_agent(concat!("agent-trace/", env!("CARGO_PKG_VERSION")))
        .build()
