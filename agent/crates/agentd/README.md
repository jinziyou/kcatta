...
# 带 --upload：本地产出 + 上报 analyzer（collect-host/collect-trace 产出后 POST；respond 注入 analyzer sink 实时推送）
agentd collect-host  -r / --malware --upload http://127.0.0.1:10068   # → /ingest/asset-report
agentd collect-trace capture --upload http://127.0.0.1:10068          # → /ingest/trace-batch
agentd respond --upload http://127.0.0.1:10068 --stdout         # → /ingest/guard-event（常驻推送）

cargo run -p agentd --features full -- respond --stdout   # 开启 onaccess/network/ids/pcap
```

feature：`pcap`（→ collect-trace / respond 网络抓包后端）、`onaccess`、`network`、`ids`、`full`，转发到对应能力
crate。eBPF 后端是各能力 crate 自身的 `ebpf` feature（构建期 nightly + bpf-linker、运行期需特权），
opt-in 且不进 musl 部署构建；缺失工具链/权限时优雅回退（trace→pcap/mock，respond→nft）。
...
- `guard.enabled` 时，respond 在后台线程常驻运行，实时把 `GuardEventBatch` 推送 analyzer
  （复用 `agentd respond --upload` 那条注入 sink 的通道）；
...
> ③ 由 analyzer 的 `analyzer-scan --capability {host|trace|guard}` 远程调度（host/trace 投精简 bin + 拉回入库；guard 投本 `agentd` 二进制并 `agentd respond --upload` 常驻推送）。
