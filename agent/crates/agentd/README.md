...
- 每隔 `interval_secs`（默认 300s）调度一次 SOC 周期：host 显式执行
  `FilesystemSource/run_scan_at_with_opts`（默认同时采集容器 rootfs 与本地镜像包，可配置限额或关闭）
  → `agent_detect::host::detect`（detect）→ 合并
...
```json
{
  ...
  "trace": { "enabled": true, "backend": "winnet", "intel": null },
  "guard": { "enabled": false, "config_path": "/etc/kcatta/guard.json" }
}
```

trace 缺省关闭；显式启用但未填写 backend 时选择真实 `winnet`。`trace.backend` 可为
`mock` / `pcap` / `ebpf` / `winnet`；`mock` 仅供显式开发/演示，生产默认不会上传合成事件。后三者需对应 build feature，缺 feature
