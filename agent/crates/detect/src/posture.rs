...
/// Contain a path under `scan_root` (drop `/`, `.`, `..`).
fn resolve_under_root(scan_root: &Path, path: &str) -> PathBuf {
    let path = path.trim();
    if path.is_empty() {
...
/// Collect all posture findings under `scan_root`, host-attributed to `host_id`.
pub fn collect(scan_root: &Path, host_id: &str) -> Vec<Vulnerability> {
    let (mut out, permit_empty_passwords) = sshd::evaluate(scan_root);
    out.extend(shadow::evaluate(scan_root, permit_empty_passwords));
    out.extend(suid::evaluate(scan_root));
    for v in &mut out {
...
mod sshd {
    ...
    pub(super) fn evaluate(scan_root: &Path) -> (Vec<Vulnerability>, bool) {
        let top = join_root(scan_root, "etc/ssh/sshd_config");
        if !top.exists() {
            return (Vec::new(), false); // no sshd configured -> nothing to say
        }
        let mut directives = Vec::new();
        flatten_file(scan_root, &top, 0, &mut directives);
    ...
    fn flatten_file(scan_root: &Path, file: &Path, depth: usize, out: &mut Vec<Directive>) {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            if keyword_lc == "include" {
                for pattern in args.split_whitespace() {
                    for included in resolve_includes(scan_root, pattern) {
                        flatten_file(scan_root, &included, depth + 1, out);
                    }
                }
                continue;
            }
    ...
    fn resolve_includes(scan_root: &Path, pattern: &str) -> Vec<std::path::PathBuf> {
        let logical = if pattern.starts_with('/') {
            pattern.to_string()
        } else {
            format!("etc/ssh/{pattern}")
        };
        let resolved = resolve_under_root(scan_root, &logical);
...
mod shadow {
    ...
    pub(super) fn evaluate(scan_root: &Path, permit_empty_passwords: bool) -> Vec<Vulnerability> {
        let Ok(shadow_text) = fs::read_to_string(join_root(scan_root, "etc/shadow")) else {
            return Vec::new(); // unreadable (unprivileged) or absent -> say nothing
        };
        let passwd = fs::read_to_string(join_root(scan_root, "etc/passwd")).unwrap_or_default();
...
mod suid {
    ...
    #[cfg(unix)]
    pub(super) fn evaluate(scan_root: &Path) -> Vec<Vulnerability> {
        use std::os::unix::fs::MetadataExt;

        let mut out = Vec::new();
        for dir in BIN_DIRS {
            walk(
                &join_root(scan_root, dir),
                scan_root,
                0,
                &mut |path, meta| {
                    let mode = meta.mode();
    ...
    #[cfg(not(unix))]
    pub(super) fn evaluate(_scan_root: &Path) -> Vec<Vulnerability> {
