//! posture-host: host static file detection (the 主机静态文件检测 capability).
//!
//! Reads a mounted filesystem root and produces either per-asset JSON files
//! (`-o DIR`) or a merged [`posture_host::AssetReport`] (stdout / `--report-out`
//! / `--upload`). `--malware` adds the built-in signature/hash malware scan.
//!
//! Independent binary over the `posture-host` library — links neither the flow
//! capture surface nor the guard daemon.
...
struct ScanArgs {
    ...
    #[arg(long, default_value_t = posture_host::default_workers())]
    malware_jobs: usize,
