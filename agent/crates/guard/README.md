...
posture 的**实时防护**能力：一个 crate = lib（防护引擎）+ `posture-guard` 守护进程。流水线：
...
- **检测**（Linux）：`fim`（inotify 文件完整性）、`behavior`（/proc 进程行为）默认开；
  `onaccess`（fanotify + 复用 `posture-host` 的内置查毒，需 `CAP_SYS_ADMIN`）、`network`/`ids`
  （复用 `posture-flow` 捕获 + `ThreatFeed` IOC 匹配）为可选 feature。
...
guard 经 feature 可选依赖 `posture-host`（onaccess）/ `posture-flow`（network），默认不牵入。

```bash
cargo run -p posture-guard -- --stdout                # 默认 monitor（FIM+行为），只写本地，无需 root
cargo run -p posture-guard -- --config /etc/posture/guard.json --stdout   # 独立 bin：本地 NDJSON/stdout，不上报
cargo run -p posture-agent -- guard --upload http://127.0.0.1:8000 --stdout   # 上报经统一 agent
cargo test -p posture-guard --features all            # 流水线 + 安全 + 全传感器（无需 root）
cargo build -p posture-guard --no-default-features --features fim   # 精简：仅 FIM
cargo build -p posture-guard --features all           # 全机制（+pcap 需 libpcap-dev）
```
