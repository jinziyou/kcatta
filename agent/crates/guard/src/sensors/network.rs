...
//!
//! Reuses `posture-flow`'s capture + `ThreatFeed` IOC matcher: each iteration
...
/// Network / IDS sensor driving `posture-flow` capture in a loop.
...
#[cfg(feature = "ids")]
fn ids_check(flow: &posture_flow::FlowEvent) -> Option<Detection> {
