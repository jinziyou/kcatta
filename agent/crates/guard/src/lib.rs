//! posture-guard library: real-time protection engine.
//!
...
//! - `onaccess` — fanotify + built-in signature scanner (reuses `posture-host`'s
//!   `malware` module; needs `CAP_SYS_ADMIN`)
//! - `network` / `ids` — `posture-flow` capture + `ThreatFeed` IOC matching
//!
