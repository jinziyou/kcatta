...
agent 的**编排二进制**：唯一入口 `agent`，通过子命令把各能力域的进程内模块调度起来——主机资产扫描、网络流捕获、IOC 情报同步。本 crate 自身不实现检测逻辑，只负责参数解析、调度与（可选的）上报；具体采集分别落在 [`agent-host`](../host)、[`agent-flow`](../flow)，上报落在 [`agent-ingest`](../ingest)，数据契约见 [`agent-contract`](../contract)。

> 边界：agent **只采集**本机/目标机上的检测工具集；CVE 判定与跨源关联在 **fusion** 侧。**跨机投放/调用/取回**（上传到待测机器、调用 `agent`、取回结果）由 fusion 侧的 `fusion-scan`（Python）负责，不再属于 agent——`agent-runtime` 只调度本机/目标机上的进程内模块。

## 二进制 `agent`

来自本 crate（bin 名 `agent`），三个子命令：

| 子命令 | 职责 | 域模块 |
| --- | --- | --- |
| `agent host` | 主机资产扫描（静态资产发现 + 可选 ClamAV 查杀） | [`agent-host`](../host) |
| `agent flow` | 网络流捕获 → IOC 匹配 → `FlowBatch` | [`agent-flow`](../flow) |
| `agent intel-sync` | 下载 IOC feed → 本地 JSON（供 `agent flow --intel` 读取） | [`agent-flow`](../flow) |
...
- **合并**：不带 `-o` → 装配 collector 计划成单个 [`AssetReport`](../contract/src/lib.rs)，输出到 stdout（`--pretty`）/ 文件（`--report-out FILE`）/ fusion（`--upload URL`）；`--malware` 时 ClamAV 命中并入 `vulnerabilities`。
...
| `--upload <URL>` | 合并 `AssetReport` 上报 fusion（`<URL>/ingest/asset-report`） |
...
| `--upload <URL>` | 捕获后上报 fusion（`<URL>/ingest/flow-batch`） |
| `--mock` | 合成 mock 流量（默认） |
...
下载 IOC feed → 本地 JSON。沿用 `fusion-osv-sync` 的离线刷新模型：sync 是显式、可调度的一步，`agent flow --intel` 只读盘上的 feed。
...
| `host` | 启用 `agent host`，引入 `agent-host` |
| `flow` | 启用 `agent flow` / `agent intel-sync`，引入 `agent-flow` + `reqwest` |
| `malware` | 在 host 之上追加 ClamAV（`agent-host/malware`） |
| `pcap` | 在 flow 之上启用 libpcap 实时抓包（`agent-flow/pcap`） |
| `full` = `[host, flow, malware]` | 全功能 |

**精简主机 agent**：仅做主机扫描时只取 host（+可选 malware），不牵 flow/pcap/HTTP，产物为单一 `agent` 二进制：
...
DAG 汇点 [`agent-contract`](../contract)（数据契约，零内部依赖）↑；本 crate 位于依赖图顶端，无任何 crate 反向依赖它。

| 依赖 | 类型 | 用途 |
| --- | --- | --- |
| [`agent-contract`](../contract) | 必选 | `AssetReport` / `FlowBatch` 数据契约 |
| [`agent-ingest`](../ingest) | 必选 | 上报 fusion（`--upload`） |
| [`agent-host`](../host) | 可选（`host`） | 主机资产采集与扫描调度（`Collector` / `ScanContext` / `run_scan_at*` / `run_static_scan`） |
| [`agent-flow`](../flow) | 可选（`flow`） | 流量捕获 + IOC 匹配 + IOC feed 同步适配器 |
