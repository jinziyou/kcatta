//! `agent host`: host asset detection.
//!
//! Two output modes:
...
//!   the mode `fusion-scan` drives on a target.
//! - **merged**: no `-o` → assemble a collector plan into a single
//!   [`agent_contract::AssetReport`] (stdout / `--report-out` / `--upload`).
...
pub struct HostArgs {
    ...
    #[cfg(feature = "malware")]
    #[arg(long, default_value_t = agent_host::default_workers())]
    malware_jobs: usize,
...
fn build_plan(
    ...
    let mut plan: Vec<Box<dyn Collector>> = agent_host::default_collectors();

    #[cfg(feature = "malware")]
    if args.malware {
        let mut malware = agent_host::MalwareCollector::default().with_workers(args.malware_jobs);
        if let Some(sock) = &args.clamd_socket {
            malware = malware.with_address(agent_host::malware::ClamdAddress::Unix(sock.clone()));
        }
...
fn run_static_malware(
    ...
    use agent_host::malware::{detection_to_vulnerability, run_scan, ClamdAddress, MalwareOptions};
