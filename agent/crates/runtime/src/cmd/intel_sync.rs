//! `agent intel-sync`: download remote IOC feeds to local JSON.
//!
//! Mirrors the offline-friendly refresh model of `fusion-osv-sync`: sync is an
//! explicit, schedulable step; `agent flow --intel` reads the on-disk feed
//! only. The HTTP download lives here; the flow library's `intel::sync` adapters
//! only parse bytes.

use std::path::PathBuf;
use std::time::Duration;

use agent_flow::intel::sync::{self, feodo};
use agent_flow::ThreatFeed;
...
pub fn run(args: IntelSyncArgs) -> Result<()> {
