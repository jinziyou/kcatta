...
**只有 `agent <cap> --upload <URL>` 才把结果上报 analyzer**。
...
# 带 --upload：本地产出 + 上报 analyzer（host/flow 产出后 POST；guard 注入 analyzer sink 实时推送）
...
> ③ 由 analyzer 的 `analyzer-scan --capability {host|flow|guard}` 远程调度（host/flow 投精简 bin + 拉回入库；guard 投本 `agent` 二进制并 `agent guard --upload` 常驻推送）。
