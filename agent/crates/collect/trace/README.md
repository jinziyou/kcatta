...
kcatta 的 **collect/trace** 能力：一个 crate = lib（网络捕获 + feed 解析；IOC 匹配在
`agent-detect::ioc`，经 `enrich_batch` / re-export `ThreatFeed`；被 guard network 复用；可选 eBPF）
+ `agent-collect-trace` 二进制。产出 [`TraceBatch`](../contract/src/lib.rs)。

编排两步：`capture_batch`（collect）→ `enrich_batch`（detect）。CLI 默认两步；`--no-intel` 只采集。
...
注入网络流事件，CVE 判定 / 跨源关联在 **analyzer** 侧；上报由统一 `agentd collect-trace --upload`
（或 `agentd run`）负责。lib **不含 reqwest**：`intel-sync` 的 feed HTTP 下载在 bin 的 `cli` 里
（本地 `http_get_text`），feed 字节解析在 lib 的 `intel::sync`。

## 子命令

- `capture` — `capture_batch` →（默认）`enrich_batch` → `TraceBatch`。`--no-intel` 跳过 IOC。
...
cargo run -p agentd -- collect-trace --upload http://127.0.0.1:10068 capture   # 上报经统一 agentd
