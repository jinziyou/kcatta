...
fn attach_optional(ebpf: &mut Ebpf, prog: &str, category: &str, name: &str) {
    if let Err(e) = attach(ebpf, prog, category, name) {
        eprintln!(
            "agent-collect-trace: optional tracepoint {category}/{name} unavailable ({e}); continuing"
        );
    }
}
...
pub fn capture(
    ...
    match read_dropped(&ebpf) {
        Ok(0) => {}
        Ok(n) => eprintln!(
            "agent-collect-trace: eBPF ring buffer overflow — {n} event(s) dropped; \
             increase the capture window or reduce load to avoid blind spots"
        ),
        Err(e) => eprintln!("agent-collect-trace: could not read eBPF drop counter: {e}"),
    }
