...
//! extracts the fields needed to populate `TraceEvent` (5-tuple, byte counts,
...
mod tests {
    ...
    fn parses_dns_query() {
        ...
        assert_eq!(pkt.proto, TraceProto::Udp);
    ...
    fn parses_tls_sni_and_ja3() {
        ...
        let pkt = parse_frame(&frame).expect("parse");
        assert_eq!(pkt.proto, TraceProto::Tcp);
