//! Scan orchestration: assemble **asset** [`Collector`]s into an [`AssetReport`].
//!
//! ## Two scan APIs (intentional)
//!
//! | API | Module | Output | Used by |
//! | --- | --- | --- | --- |
//! | [`run_scan_at`] / [`run_scan_with_detect`] | this module | merged [`AssetReport`] | `agentd run`, merged CLI |
//! | [`crate::run_static_scan`] | [`crate::scan`] | per-file JSON (`-o DIR`) | static CLI / analyzer-scan pull |
//!
//! Both share collectors / sources; keep both until a single facade can preserve
//! the two output shapes without breaking deploy consumers.
//!
//! **Collect vs detect**: [`run_scan_at`] runs asset collectors only.
//! [`run_scan_with_detect`] = collect then [`crate::run_detect_at`] (malware /
//! posture / secrets). Prefer that over stuffing detect adapters into the
//! collector plan.
//!
//! # Example
//!
//! ```no_run
//! use agent_collect_host::{default_collectors, run_scan_at};
//!
//! let report = run_scan_at(&default_collectors(), "/")?;
//! # Ok::<(), anyhow::Error>(())
//! ```

use agent_contract::AssetReport;
use chrono::Utc;
use uuid::Uuid;

use crate::collector::{Collector, CollectorOutput, ScanContext, WindowsPackageProfile};
use crate::detect_phase::{run_detect_at, DetectOpts};

/// Run collectors at the live host root (`/`).
pub fn run_scan(collectors: &[Box<dyn Collector>]) -> anyhow::Result<AssetReport> {
    run_scan_at(collectors, "/")
}

/// Run collectors against `scan_root` (mounted filesystem or `/`).
///
/// Callers must include a host collector first so asset collectors receive
/// `host_id` in [`ScanContext`].
pub fn run_scan_at(
    collectors: &[Box<dyn Collector>],
    scan_root: impl AsRef<std::path::Path>,
) -> anyhow::Result<AssetReport> {
    run_scan_at_with(collectors, scan_root, Vec::new())
}

/// Like [`run_scan_at`], but also passes extra project roots (relative to
/// `scan_root`) for language-package collectors to scan.
pub fn run_scan_at_with(
    collectors: &[Box<dyn Collector>],
    scan_root: impl AsRef<std::path::Path>,
    project_roots: Vec<std::path::PathBuf>,
) -> anyhow::Result<AssetReport> {
    run_scan_at_with_opts(
        collectors,
        scan_root,
        project_roots,
        WindowsPackageProfile::default(),
    )
}

/// Like [`run_scan_at_with`], but also sets the Windows package collection scope.
pub fn run_scan_at_with_opts(
    collectors: &[Box<dyn Collector>],
    scan_root: impl AsRef<std::path::Path>,
    project_roots: Vec<std::path::PathBuf>,
    windows_packages: WindowsPackageProfile,
) -> anyhow::Result<AssetReport> {
    let mut ctx = ScanContext::at(scan_root)
        .with_project_roots(project_roots)
        .with_windows_packages(windows_packages);
    let mut assets = Vec::new();

    for collector in collectors {
        match collector.collect(&mut ctx)? {
            CollectorOutput::Host(host) => {
                ctx.host_id = Some(host.host_id.clone());
                ctx.host = Some(host);
            }
            CollectorOutput::Assets(batch) => assets.extend(batch),
        }
    }

    let host = ctx
        .host
        .ok_or_else(|| anyhow::anyhow!("scan plan must include a host collector"))?;

    Ok(AssetReport {
        report_id: format!("report-{}", Uuid::new_v4()),
        collected_at: Utc::now(),
        scanner_version: env!("CARGO_PKG_VERSION").to_string(),
        host,
        assets,
        vulnerabilities: Vec::new(),
    })
}

/// Asset collect ([`run_scan_at_with_opts`]) then optional detect phase.
///
/// `detect` may be empty ([`DetectOpts::default`]) for assets-only.
pub fn run_scan_with_detect(
    collectors: &[Box<dyn Collector>],
    scan_root: impl AsRef<std::path::Path>,
    project_roots: Vec<std::path::PathBuf>,
    windows_packages: WindowsPackageProfile,
    detect: &DetectOpts,
) -> anyhow::Result<AssetReport> {
    let scan_root = scan_root.as_ref();
    let mut report =
        run_scan_at_with_opts(collectors, scan_root, project_roots, windows_packages)?;
    if detect.any_enabled() {
        let findings = run_detect_at(scan_root, &report.host.host_id, detect)?;
        report.vulnerabilities.extend(findings);
    }
    Ok(report)
}
