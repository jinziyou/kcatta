...
- **检测**（Linux）：`fim`（inotify 文件完整性）、`behavior`（/proc 进程行为）默认开；
  `onaccess`（fanotify + 复用 `agent-collect-host` 的内置查毒，需 `CAP_SYS_ADMIN`）、`network`/`ids`
  （复用 `agent-collect-trace` 捕获 + `ThreatFeed` IOC 匹配）为可选 feature。
...
guard 经 feature 可选依赖 `agent-collect-host`（onaccess）/ `agent-collect-trace`（network），默认不牵入；
...
cargo run -p agentd -- respond --upload http://127.0.0.1:10068 --stdout   # 上报经统一 agent
