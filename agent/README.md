...
- **x86_64**：需 musl C 工具链（posture-host 的内置 SQLite、TLS 的 ring 走 C/asm）：Debian/Ubuntu `sudo apt-get install -y musl-tools`；纯 Rust 子集（如 `posture-guard --features fim`）无需。
...
- **CI**：`agent (musl deploy build)` 与 `agent (musl deploy build, arm64)` 两个 job 分别构建并上传 `posture-agent-musl-x86_64` / `posture-agent-musl-aarch64` 制品。
...
cargo run -p posture-agent -- host -r / -t all --upload http://127.0.0.1:8000   # 上报 fusion（统一 agent）
```
...
cargo run -p posture-agent -- flow --upload http://127.0.0.1:8000 capture --intel data/feeds/feodo.json
