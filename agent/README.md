...
1. **三独立二进制**（最精简）：`posture-host` / `posture-flow` / `posture-guard` 各自单独构建、部署、运行。
2. **统一 `agent` 命令**（umbrella）：单一二进制 `agent`，子命令 `host` / `flow` / `guard` 在进程内分发到三能力（见 [`crates/agent`](crates/agent)），与三独立二进制共用同一套 CLI 逻辑（各能力 lib 的 `cli` 模块）。
3. **由 fusion 调度**：`fusion-scan --capability {host|flow|guard}` 经 SSH 远程投放——host/flow 一次性拉回结果，guard 部署为常驻守护并推送 fusion。

```bash
posture-host -r / --malware --pretty                       # 方式1：独立二进制
agent host -r / --malware --pretty                         # 方式2：统一命令（等价）
fusion-scan --ssh-host root@H --capability host -o out --upload http://fusion:8000   # 方式3：fusion 调度
fusion-scan --ssh-host root@H --capability guard --upload http://fusion:8000          #       （guard 常驻）
```
...
└── agent/        # posture-agent：统一 `agent` 命令（umbrella，host/flow/guard 子命令）
```

**依赖方向**（单向无环；capability crate 互为 lib 依赖，umbrella 聚合三者）：

```
contract ← ingest / host / flow
contract ← guard ← host(onaccess, 复用内置查毒) + flow(network, 复用 capture)
cli-common（无内部依赖）
agent(umbrella) → host + flow + guard（各自的 cli 模块）
```
