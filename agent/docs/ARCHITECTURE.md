...
kcatta 的端点组件。agent 分为**三大能力**，**一个能力 = 一个目录 = 一个 crate**
（lib + bin 同处一个 crate，无嵌套子 crate），各能力可单独部署、单独运行；三者共享数据
契约与上报底座。

| 能力 | crate（目录） | 观测对象 / envelope | 边界 |
| --- | --- | --- | --- |
| **主机静态文件检测** | `agent-host`（`host/`） | 内视 → `AssetReport` | 只采集 |
| **流量检测** | `agent-flow`（`flow/`） | 外视 → `FlowBatch` | 只采集 |
| **实时防护** | `agent-guard`（`guard/`） | 端内实时事件 → `GuardEventBatch` | **检测 + 端上主动处置 + 上报** |

> CVE 判定与跨源关联在 **analyzer** 侧；`agent-host` / `agent-flow` 只产出标准化 envelope。
> **`agent-guard` 是唯一会在端上主动处置的能力**（可逆隔离 / 网络阻断 / 阻断打开），默认
> monitor 关闭、受多重安全否决保护。跨机投放（`analyzer-scan`，Python）属于 analyzer。

```
  [周期] 主机静态扫描 ──► agent-host  → host 域 (+--malware 内置查毒)         ──► 写 AssetReport 文件
  [周期|持续] 流量      ──► agent-flow  → flow 域 (mock | pcap) + intel-sync   ──► 写 FlowBatch 文件
  [持续] 实时防护       ──► agent-guard → guard 域 (fim/onaccess/behavior/network/ids) ──► 本地 NDJSON/stdout
                            └──── 独立运行：只产出本地结果，不上报 ────┘
                                              │
            agent <cap> --upload  ──►  agent 内置 ingest  ──►  /ingest/{asset-report, flow-batch, guard-event}  →  analyzer
```
...
底座:  agent-contract     (AssetReport + FlowBatch + GuardEventBatch + 共享 Severity/IndicatorType, 零内部依赖)
       ...
       agent-contract ◄── agent-flow      (capture + IOC 匹配 + feed 解析; lib 无 reqwest, 只写文件)
       agent-contract ◄── agent-guard ◄── agent-host(onaccess, 复用 malware) + agent-flow(network, 复用 capture)
       agent (umbrella, crates/agent) ◄── agent-host + agent-flow + agent-guard + agent-contract
                          └── 内置 ingest 模块 (reqwest): --upload 时 upload_report/batch/guard_batch → analyzer
```

> `cli-common` / `agent-ingest` 已移除：JSON 输出 / HTTP 下载内联进各能力 `cli`；上报（ingest）内置进 `agent`。
> **只有 `agent <cap> --upload` 上报**：host/flow 把 `run` 返回的 envelope POST；guard 由 agent 注入一个 analyzer `ReportSink`。
...
- **命名**：底座库 `agent-*`，能力 crate 用产品名 `kcatta-*`（lib 名 `agent_host`/`agent_flow`/`agent_guard`，bin 同名）；umbrella `agent`（bin `agent`）。

## 三种运行方式

1. **三独立二进制**：`agent-host` / `agent-flow` / `agent-guard` 各自单独构建、部署、运行（最精简，按 feature 裁剪）。
2. **统一 `agent` 命令**：`crates/agent` 产出单一 `agent` 二进制，子命令 `host`/`flow`/`guard` 在进程内分发到各能力 lib 的 `cli` 模块（与独立 bin 共用逻辑）。
3. **analyzer 调度**：`analyzer-scan --capability {host|flow|guard}` 经 SSH 远程投放——host/flow 一次性拉回结果，guard 部署为常驻守护推送 analyzer。
...
| `network` / `ids` | | 复用 `agent-flow` 短窗捕获 + `ThreatFeed` 匹配 / 内置端口签名 | `NetworkEvent` / `IdsEvent` |
...
`--config /etc/kcatta/guard.json`（缺失则用默认）。字段：`mode`、`host_id`、各传感器开关与路径（`onaccess.signatures` 加载额外查毒签名）、`response`（`allow_quarantine`/`allow_netblock`、`severity_threshold`、`critical_paths`、`vault_dir`）、`report`（`audit_log`/`stdout`/`batch_max`/`flush_secs`）。CLI `--detect-only` / `--stdout` 覆盖配置（`--upload` 仅存在于 `agent guard`，由 agent 注入 analyzer sink）。
...
`analyzer/schemas-json/` 的 Rust 镜像，零内部依赖，持有三种 envelope：
...
cargo run -p agent -- host -r / -t all --upload http://127.0.0.1:8000
...
cargo build -p agent-flow --no-default-features
