...
| `agent` / `agent` | umbrella：`agent host`/`flow`/`guard` 分发到各能力 `cli`；**内置 ingest**（`src/ingest.rs`），`--upload` 才上报 fusion。 |
...
**原则**：`agent-host` / `agent-flow` 只采集；CVE 判定与跨源关联在 fusion 侧。
**`agent-guard` 是唯一会端上主动处置的能力**（默认 monitor、受安全否决保护）。跨机投放由
fusion 的 `fusion-scan`（Python）负责（投放 `agent-host`）。
