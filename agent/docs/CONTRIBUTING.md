...
| `ebpf` / `agent-ebpf` | 合一 eBPF crate（许可 `Apache-2.0 AND GPL-2.0`）：一个 lib target（lib 名 `agent_ebpf`，Apache，no_std）暴露共享 `#[repr(C)]` POD 事件结构（`ExecEvent`/`ExitEvent`/`FileEvent`，bytemuck `Pod`），经 ring buffer 内核→用户态传递；两个 bpf-target bin（GPL，均 `required-features = ["ebpf"]`）：`trace-ebpf` 内核 tracepoint 程序（`trace_exec`/`trace_exit`/`trace_openat` → `EVENTS` RingBuf），`guard-ebpf` 内核 `cgroup_sock_addr` 程序（`guard_connect4`/`guard_connect6`，按 `BLOCKED_V4`/`V6` 拒绝目的 IP，不用共享 lib 类型）。workspace 成员但**不在 `default-members`**（bin 仅 bpf target）。 |
...
`ebpf` 时依赖 `agent-ebpf`（lib，共享事件类型）并经 build.rs 嵌入 `trace-ebpf` bin，`agent-guard` 经
build.rs 嵌入 `guard-ebpf` bin（不依赖 `agent-ebpf` lib 类型）。
...
`cargo test` 不会编译它们（仅在 `agent-trace --features ebpf` 时把共享 lib 透传宿主编译）。两个 bin 仅在
`agent-trace`/`agent-guard` 开启 `ebpf` feature 时，由对应 crate 的 build.rs 编译：
