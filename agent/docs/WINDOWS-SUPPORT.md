...
| **主机静态采集 `agent-collect-host`** | ✅ **已支持**（有原生 Windows 实现） | `agent-collect-host.exe` 采包/用户 profile 等 → `AssetReport` |
...
- **eBPF crate 用 Aya**（`crates/ebpf/Cargo.toml`）：内核侧 `aya-ebpf` 编译出 GPL-2.0 的 BPF 程序
  （`trace-ebpf` / `guard-ebpf` 两个 bin），用户态由 `aya 0.13`（`agent-collect-trace` / `agent-respond` 的
  `ebpf` feature）经 **Linux `bpf(2)` 系统调用**加载。Aya 整条链只在 Linux 上工作。
- **trace 的 eBPF 后端**：`CaptureBackend::Ebpf`（cgroup-skb 流量遥测，`crates/collect/trace/src/capture/mod.rs`）
  仅在 `feature = "ebpf"` 下存在，而 `aya` 依赖本身在非 Linux 上构建不出来。
- **guard 全部常驻传感器**显式 `#[cfg(all(target_os = "linux", feature = "..."))]`
  （`crates/respond/src/sensors/mod.rs`）：
  - FIM → inotify
  - behavior → 轮询 `/proc`
  - onaccess → fanotify + agent-collect-host 签名扫描
  - network → cgroup-connect eBPF
  其内核依赖 `nix`（fanotify / inotify / signal）更是写在
  `[target.'cfg(target_os = "linux")'.dependencies]`（`crates/respond/Cargo.toml`）下。
...
### 2.1 静态资产采集：已支持（agent-collect-host 有原生 Windows 实现）

- `crates/collect/host/src/platform/windows/`（如 `boot.rs` 经 `windows_sys` 调 `GetTickCount64` 取启动时间）
- `crates/collect/host/src/walk/handlers/ssh_home.rs::scan_windows_profiles`（扫 Windows 用户 profile / SSH home，跳过系统账号）
- `--windows-packages full|apps` 包采集 profile（`crates/collect/host/src/cli.rs`、`collector.rs`）

即 `agent-collect-host.exe` 可在 Windows 上跑出标准 `AssetReport`（包 / 服务 / 账号 / 凭据线索 …），
...
| onaccess 查毒 | minifilter + 复用 agent-collect-host 签名扫描器 | 是 |
