...
| misconfig/secret 建模为新 Asset | 它是 **finding 不是 inventory**——建成 Asset 会绕过 correlation/dedup/告警。✅ **已按推荐路径落地**（见 §2）：扩展 `Vulnerability`（`source='posture'`，零契约改动），sshd_config + shadow + SUID 三项确定性低误报；**未**建成新 Asset。secret 部分仍待做。 |
