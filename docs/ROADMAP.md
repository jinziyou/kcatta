...
CI1/CI4/CI2/CI3 + CI5 + Q4/Q5/Q6 均已落地——主链路闭环，攻击面已点亮，检测覆盖/镜像归因补上，API 面也纳入
漂移门禁。剩余建议：

- **检测覆盖/契约（剩余速赢）**：**Q7 guard 受保护进程策略化**（消除升级时 SIGKILL nginx/postgres 自我 DoS，
  Rust/S）。（Q2「GHSA 接入」需先核实当前 sync 的 ecosystem 集合：OSV 的 per-ecosystem 导出已内含 GHSA，
