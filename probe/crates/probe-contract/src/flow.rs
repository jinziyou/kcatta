//! Network flow contract — the `probe-flow` (network probe) envelope.
//!
//! Mirrors `form.schemas.flow` / `form.schemas.threat`. These types live here
//! alongside the host [`AssetReport`](crate::AssetReport) contract so the
//! shared `probe-ingest` client can serialize both host and network telemetry,
//! and so a single crate is the Rust mirror of `form/schemas-json/`.
...
/// One IOC hit observed on a flow by network-probe preliminary processing.
/// Mirrors `form.schemas.threat.ThreatMatch`.
...
pub struct FlowEvent {
    ...
    /// IOC matches found by network-probe preliminary processing.
    /// Serialized as `[]` when empty so the field is always present.
    #[serde(default)]
    pub threat_intel: Vec<ThreatMatch>,
}

/// network probe -> form: a batch of flow events from one probe instance.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FlowBatch {
    ...
    /// Id of the probe instance that produced the batch.
    pub collector_id: String,
    /// Version string of the probe that produced the batch.
