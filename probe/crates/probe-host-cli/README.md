...
# 写入文件
cargo run -p probe-host-cli -- -r / --out report.json

# 静态分文件模式（等同 probe-asset）
cargo run -p probe-host-cli -- -r / -t all --asset-out ./scan-out
...
- 指定 `--asset-out` 时进入 **静态分文件模式**，行为与 `probe-asset` 一致，不输出合并 `AssetReport`。
