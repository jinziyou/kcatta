...
//!
//! The rest of probe-remote uses OpenSSH's ControlMaster pipeline with
...
//!    (default `~/.config/scdr/probe-remote/keys/<user>@<host>-<port>.ed25519`).
...
pub fn default_key_path(user: &str, host: &str, port: u16) -> anyhow::Result<PathBuf> {
    ...
    let dir = base.join("scdr/probe-remote/keys");
...
fn generate_keypair(path: &Path, user: &str, host: &str) -> anyhow::Result<()> {
    ...
    let out = Command::new("ssh-keygen")
        ...
        .arg(path)
        .arg("-C")
        .arg(format!("probe-remote@{user}@{host}"))
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .output()
