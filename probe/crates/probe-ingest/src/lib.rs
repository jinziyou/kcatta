//! probe-ingest: push probe telemetry JSON to form over HTTP.
//!
//! One blocking client shared by both probe envelopes:
//! - [`upload_report`] — host [`AssetReport`] -> `/ingest/asset-report`
//! - [`upload_batch`]  — network [`FlowBatch`] -> `/ingest/flow-batch`
//!
//! Used by `probe-host-cli`, `probe-remote`, and `probe-flow-cli`. Both
//! endpoints expect form to respond with `202 Accepted` on success, and pick
//! up a bearer token from `FORM_API_TOKEN` when present.

use std::time::Duration;

use probe_contract::{AssetReport, FlowBatch};
use serde::Serialize;
