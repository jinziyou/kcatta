...
**检测全链路（主动触发，闭环）**：admin `/targets` 注册目标 + `/scans` 触发 → analyzer `POST /scans` 建异步作业、复用 deploy 层投放 agent（host/flow 一次性拉回、guard 常驻 `agent guard --upload`）→ agent 采集并（经 analyzer 入库路径）落 `AssetReport`/`FlowBatch`/`GuardEventBatch` + CVE/查毒检测与 IOC 关联 → admin `/scans/[jobId]` 轮询状态并查看本次结果（`/reports`、`/vulnerabilities`、`/flows`、`/guard`）。凭据为 analyzer 主机上的托管 SSH 密钥（注册时一次性密码 bootstrap 后丢弃，不持久化）。
...
命令（umbrella，子命令 `host`/`flow`/`guard`，`--upload` 时上报 analyzer）；③ 由 **analyzer**
...
analyzer 接收 agent-flow 上报的、已带威胁情报命中（`FlowEvent.threat_intel`）的 `FlowBatch`，
