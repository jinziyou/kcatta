...
- **主机静态文件检测（`posture-host`）**：本机 / 挂载目录静态扫描（包、SBOM、服务、账户、SSH 指纹）+ 内置签名查毒，产出 `AssetReport`（CVE 判定交给 fusion）。
- **流量检测（`posture-flow`）**：流量元数据采集 + 威胁情报 IOC 匹配与情报库同步，产出 `FlowBatch`。
- **实时防护（`posture-guard`）**：长驻守护（FIM / on-access 查毒 / 进程行为 / 网络 IOC / IDS 实时检测），可选端上主动处置（默认全关），产出 `GuardEventBatch`。
