...
> `package.json` 定义 `dev / build / start / typecheck / lint / generate:contracts / test:e2e`。`pnpm test:e2e` 在本地会复用已经起好的 admin/analyzer 服务，CI 下由 `scripts/e2e-admin.sh` / `scripts/e2e-analyzer.sh` 自动拉起（见 `playwright.config.ts`）。
